...
@section('title', 'WG Absen — Laporan Makan')
            ...
            <div class="filter-group">
                ...
                <select id="filterBiro" class="filter-select" placeholder="-- Please select --">
                    ...
                    @foreach($biroList as $b)
                        <option value="{{ $b->id }}">{{ $b->biro_name }}</option>
                    @endforeach
                </select>
            </div>
...
<script>
    ...
    function openUangMakanModal() {
        ...
        fetch('{{ route("makan.getUangMakan") }}')
    ...
    function simpanUangMakan() {
        ...
        fetch('{{ route("makan.updateUangMakan") }}', {
            method: 'POST',
            headers: {
