...
<body>
  ...
  <script>
    $(document).ready(function() {
      ...
      const checks = {
        length: {
          el: document.getElementById('check-length'),
          icon: document.getElementById('icon-length'),
          test: (p) => p.length >= 8
        },
        lowercase: {
          el: document.getElementById('check-lowercase'),
          icon: document.getElementById('icon-lowercase'),
          test: (p) => /[a-z]/.test(p)
        },
        uppercase: {
          el: document.getElementById('check-uppercase'),
          icon: document.getElementById('icon-uppercase'),
          test: (p) => /[A-Z]/.test(p)
        },
        number: {
          el: document.getElementById('check-number'),
          icon: document.getElementById('icon-number'),
          test: (p) => /[0-9]/.test(p)
        },
        symbol: {
          el: document.getElementById('check-symbol'),
          icon: document.getElementById('icon-symbol'),
          test: (p) => /[!@#$%^&*()_+\-=\[\]{};':"\\|,.<>\/?`~]/.test(p)
        }
      };
