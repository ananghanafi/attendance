...
@section('title', 'WG Absen — Edit Kalender Kerja')

@section('styles')
  .card{background:#fff;border-radius:18px;border:1px solid #e7eaf3;box-shadow:0 10px 35px
    ...
    <form method="POST" action="{{ route('kalender.update', ['id' => $row->id]) }}" style="margin-top:14px">
      ...
      <div class="grid">
        ...
        <div>
          ...
          <input type="number" id="nilai_persentase" name="nilai_persentase" min="0" max="100" step="0.01" required
            value="{{ $wfoVal }}">
      ...
      <div class="row2">
        <div>
          ...
          <input type="date" id="tgl_awal" name="tgl_awal" required
            value="{{ optional($row->tgl_awal)->format('Y-m-d') }}">
        </div>
        <div>
          <label for="tgl_akhir">Tanggal Akhir</label>
          <input type="date" id="tgl_akhir" name="tgl_akhir" required
            value="{{ optional($row->tgl_akhir)->format('Y-m-d') }}">
        </div>
      </div>
...
@section('scripts')
  // Initialize bulan/tahun selects for edit form (no
  <script>
