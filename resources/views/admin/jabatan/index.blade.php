  ...
  <head>
    ...
    <style>
      ...
      *{box-sizing:border-box;font-family:Inter,ui-sans-serif,system-ui,-apple-system,'Segoe UI',Roboto,'Helvetica Neue',Arial}
      ...
      .card{background:var(--card);border-radius:10px;box-shadow:0 6px 20px rgba(20,20,60,0.06);padding:18px;min-width:0;overflow:hidden}
      ...
      th,td{padding:10px 8px;border-bottom:1px solid #eef0f6;text-align:left;font-size:14px;vertical-align:top;white-space:nowrap}
  ...
  <body>
    <div class="wrap">
      <div class="top">
        <div>
          <h1>Data Jabatan</h1>
          <div class="muted" style="font-size:12px">Tambah + edit + delete jabatan.</div>
        </div>
        <div style="display:flex;gap:10px;flex-wrap:wrap">
          @include('partials.back', ['fallback' => route('settings.jabatan')])
          <a href="{{ route('jabatan.create') }}"><button class="btn primary" type="button">Tambah Jabatan</button></a>
        </div>
      </div>
            ...
            <tbody>
              @forelse($rows as $r)
                <tr>
                  ...
                  <td>
                    <div class="actions">
                      <a href="{{ route('jabatan.edit', ['id' => $r->id]) }}"><button class="btn" type="button">Edit</button></a>
                      <form method="POST" action="{{ route('jabatan.destroy', ['id' => $r->id]) }}" onsubmit="return confirm('Yakin hapus jabatan ini?');" style="margin:0">
                        @csrf
                        @method('DELETE')
